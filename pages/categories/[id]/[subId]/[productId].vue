  ...
  meta: [
    {
      name: "description",
      content:
        "اكتشف تفاصيل المنتج، المواصفات، والمراجعات على Shatably - وجهتك للتسوق الإلكتروني.",
    },
    {
      name: "keywords",
      content:
        "تسوق إلكتروني, تفاصيل المنتج, مراجعات المنتج, مواصفات المنتج, Shatably",
    },
  ],
});
// -------------------------------------------------------
...
const fetchProduct = async () => {
  ...
  try {
    ...
    // جلب اسم الـ subcategory
    const subcategoryDoc = await getDoc(
      doc(db, "categories", categoryId, "subcategories", subId)
    );
    if (subcategoryDoc.exists() && subcategoryDoc.data().name) {
      subcategoryName.value = subcategoryDoc.data().name;
    }
...
const scrollRecentlyViewed = (direction) => {
  ...
  if (direction === "next") {
    recentlyViewedIndex.value = Math.min(
      recentlyViewedIndex.value + 1,
      maxIndex
    );
  } else {
...
const scrollRecentlyViewedMobile = (direction) => {
  const maxIndex = Math.max(0, recentlyViewedProducts.value.length - 3);
  if (direction === "next") {
    recentlyViewedIndexMobile.value = Math.min(
      recentlyViewedIndexMobile.value + 1,
      maxIndex
    );
  } else {
    recentlyViewedIndexMobile.value = Math.max(
      recentlyViewedIndexMobile.value - 1,
      0
    );
  }
...
<div class="flex items-center gap-2 text-xs md:text-sm mx-4 md:mx-6 lg:mx-10 mt-6 md:mt-8 lg:mt-10 mb-4 md:mb-6">
  ...
  <font-awesome-icon :icon="['fas', 'chevron-right']" class="text-[10px] md:text-[12px]" />
  <NuxtLink :to="`/categories/${categoryId}/${subId}`" class="text-gray-600 hover:text-[#C76950]">
  ...
  <span class="text-[#C76950] font-semibold">Product Details</span>
    ...
    <div
      v-else-if="error"
      class="bg-red-50 border border-red-200 rounded-lg p-6 text-center mx-4"
    >
    ...
    <div v-else-if="product" class="max-w-7xl mx-auto">
      <div class="hidden lg:grid lg:grid-cols-2 gap-8 px-4 py-8">
        <div class="flex flex-col gap-4">
          <div class="relative bg-white rounded-lg overflow-hidden shadow-sm">
            ...
            <button
              ...
              class="absolute top-4 right-4 bg-white w-10 h-10 rounded-full flex justify-center items-center shadow-md hover:bg-gray-100"
            >
              <font-awesome-icon
          ...
          <div class="bg-white rounded-lg px-4 py-4 shadow-sm">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="flex gap-2">
                ...
                <button
                  ...
                  :disabled="
                    recentlyViewedIndex >= recentlyViewedProducts.length - 3
                  "
                  class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center hover:bg-gray-300 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                >
                  <font-awesome-icon
            ...
            <div class="flex gap-4">
              ...
              >
                <div
                  class="bg-gray-50 rounded-lg p-3 mb-2 h-32 flex items-center justify-center"
                >
                  <img
                    :src="prod.image"
                    :alt="prod.name"
                ...
                <p class="text-xs text-gray-800 truncate mb-1">
                  {{ prod.name }}
                </p>
        ...
        <div class="flex flex-col gap-6">
          ...
          <div class="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div
              class="flex items-center gap-4 w-fit bg-gray-100 rounded-full p-3"
            >
              <button
          ...
          <div class="flex gap-3">
            <button
              @click="buyNow"
              class="flex-1 h-14 bg-[#C76950] text-white rounded-full hover:bg-[#b85840] font-semibold"
            >
              Buy Now
            </button>
            <button
              @click="addToCart"
              class="flex-1 h-14 border border-[#612B1F] text-[#612B1F] rounded-full hover:bg-[#b85840] hover:border-[#b85840] hover:text-white font-semibold"
            >
              Add to Cart
            </button>
          </div>
      ...
      <div class="lg:hidden">
        <div class="relative">
          ...
          <button
            @click="toggleWishlist"
            class="absolute top-4 right-4 bg-white w-10 h-10 rounded-full flex justify-center items-center shadow-md"
          >
            <font-awesome-icon
              :icon="[isInWishlist ? 'fas' : 'far', 'heart']"
              :class="isInWishlist ? 'text-orange-500' : 'text-gray-400'"
        ...
        <div class="px-4 py-4 space-y-4">
          <div>
            ...
            <p class="text-3xl font-bold text-[#C76950] mt-3">
              {{ product.price }} LE
            </p>
          ...
          <div>
            ...
            <div
              class="flex items-center gap-3 w-fit bg-gray-100 rounded-full px-4 py-2"
            >
              <button
                @click="decreaseQuantity"
                :disabled="quantity <= 1"
                class="text-[#C76950] disabled:opacity-50"
              >
                <font-awesome-icon :icon="['fas', 'minus']" />
              </button>
          ...
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p class="font-semibold text-blue-900 mb-1 text-sm">
              Delivery info
            </p>
          ...
          <div class="flex gap-3 pb-4">
            <button
              @click="buyNow"
              class="flex-1 h-12 bg-[#C76950] text-white rounded-full font-semibold text-sm"
            >
              Buy Now
            </button>
            <button
              @click="addToCart"
              class="flex-1 h-12 border-2 border-[#C76950] text-[#C76950] rounded-full font-semibold text-sm"
            >
              Add to cart
            </button>
          </div>
        ...
        <div class="px-4 py-5 bg-white">
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex gap-2">
              ...
              <button
                ...
                :disabled="
                  recentlyViewedIndexMobile >= recentlyViewedProducts.length - 3
                "
                class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed"
              >
                <font-awesome-icon
          ...
          <div class="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            ...
            >
              <div
                class="bg-gray-50 rounded-lg p-2 mb-2 h-28 flex items-center justify-center"
              >
                <img
                  :src="prod.image"
                  :alt="prod.name"
              ...
              <p class="text-xs font-medium truncate text-gray-800 mb-1">
                {{ prod.name }}
              </p>
      ...
      <div class="bg-gray-50 px-4 py-6 lg:py-8">
        <div class="max-w-7xl mx-auto">
          <h2 class="text-xl lg:text-3xl font-bold mb-4 lg:mb-6">
