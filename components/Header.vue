  ...
  <header class="w-full bg-gray-100 px-[80px] pb-2 flex flex-col">
    ...
    <nav class="flex items-center justify-between">
      <div class="flex items-center space-x-2">
        ...
        <img src="/logo mod.png" class="h-[auto] w-[200px] -my-6" alt="logo"></img>
      </div>
            ...
            <ul v-if="open" class="absolute top-full left-0 mt-2 w-40 bg-white border rounded shadow-lg">
              <li v-for="cat in categories" :key="cat.id">
                <NuxtLink class="block px-4 py-2 hover:bg-gray-100 hover:text-orange-500"
                  exact-active-class="font-medium  text-orange-500 bg-gray-100" :to="`/categories/${cat.id}`">
                  {{ cat.name }}
                </NuxtLink>
              </li>
            </ul>
      ...
      <div class="flex gap-3"> <!-- sigin in -->
        <div v-if="!user" class="flex gap-3">
          <button @click="signIN"
            class="px-5 py-2 rounded-[22px] w-[121px] h-[36px] bg-[#C76950] text-white hover:bg-[#AD563F]">
            Sign in
          </button>
          <button @click="signUP"
            class="px-5 py-2 rounded-[22px] w-[121px] h-[36px] border border-[#612B1F] text-[#612B1F] hover:text-[#C76950] hover:border-[#C76950]">
            Sign up
          </button>
        </div>
        <!--user show -->
        <div v-else class="relative">
          <img @click.stop="showMenu = !showMenu" :src="user.photoURL || defaultAvatar"
          ...
          <div v-if="showMenu" @click.stop class="absolute right-0 mt-3 w-40 bg-slate-50 rounded shadow-lg py-2 z-50">
            ...
            <!--to profile page -->
            <NuxtLink to="/userprofile" class="block w-full px-4 py-2 text-sm hover:bg-[#EBCDC5] text-gray-600">
              Profile
            </NuxtLink>
            <button @click="logout"
    ...
    <div class="flex  pl-32 ">
      <div class="flex -mt-12 pl-32 pr-6 items-center">
        ...
        <!-- مربع البحث -->
        <div
          class="flex items-center justify-between w-[700px] h-[45px] px-3 py-2 rounded-[24px] text-[#4B5563] bg-white"
          style="box-shadow: 0 0 4px rgba(0,0,0,0.3);">
          <font-awesome-icon :icon="['fas', 'magnifying-glass']" class="mr-2 text-[#4B5563]" />
          <input v-model="searchQuery" @keyup.enter="goSearch" type="text" placeholder="Search"
